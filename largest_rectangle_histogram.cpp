...
//loop lgao...o se n tk chalege....har point pe..minheight nikaalo wha tak ki ...
...
int largestRectangleArea(vector<int>& heights) {
    ...
    for (int i = 0; i < n; i++) {
        int j = i;
        int minheight = heights[i];  // Start with the current height

        // Expand left until the first bar
        while (j >= 0) {
            minheight = min(minheight, heights[j]);  // Update minheight
            int width = i - j + 1;  // Calculate width
            area = max(area, minheight * width);  // Update area
            j--;  // Move left
        }
    }
...
//HUM STACK ME INDEX MANAGE KRR RE THIS TIME....
    ...
    vector<int> ans(n); // Initialize the vector with size n
